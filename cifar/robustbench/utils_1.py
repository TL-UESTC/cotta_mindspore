...
from cifar.robustbench.model_zoo import model_dicts as all_models
from cifar.robustbench.model_zoo.enums import BenchmarkDataset, ThreatModel
...
####!!!!! 报错地点
    ...
    response = session.get(url_base, params={'id': gdrive_id}, stream=True, verify=False)
    token = get_confirm_token(response)

    if token:
        params = {'id': gdrive_id, 'confirm': token}
        response = session.get(url_base, params=params, stream=True, verify=False)
    ...
    with torch.no_grad():
        for counter in range(n_batches):
            ...
            y_curr = y[counter * batch_size:(counter + 1) *
                       batch_size].to(device)
            #print('x_curr', x_curr.shape) (200,3,32,32)
            # anchor_prob = torch.nn.functional.softmax(model(x_curr), dim=1).max(1)[0]
            # if anchor_prob.mean(0)<0.9:
            #     x_curr = noise(x_curr)
            # else:
            #     x_curr = x_curr
            # x_curr = noise(x_curr) #这里有问题
            output = model(x_curr)

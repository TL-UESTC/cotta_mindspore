...
def download_gdrive(gdrive_id, fname_save):
    ...
    response = session.get(url_base, params={'id': gdrive_id}, stream=True, verify=False, proxies=proxy)
    token = get_confirm_token(response)

    if token:
        params = {'id': gdrive_id, 'confirm': token}
        response = session.get(url_base, params=params, stream=True, verify=False, proxies=proxy)
...
def convert_state_dict(state_dict):
    ...
    for name, param in state_dict.items():
        msparam = Tensor(param.numpy())
        if isinstance(msparam, ms.Tensor):
            param = Parameter(msparam, name=name)
        ms_state_dict[name] = msparam
...
def load_model(model_name: str,
    ...
    model_path = model_dir_ / f'{model_name}_state_dict.pt'
    ...
    if not isinstance(models[model_name]['gdrive_id'], list):
        ...
        # 将torch.Tensor转化为ms.tensor
        for k, v in state_dict.items():
            v = Tensor(v.numpy())
            # print(type(v))
        ms_state_dict = convert_state_dict(state_dict)
        # print(ms_state_dict)
        model = _safe_load_state_dict(model, model_name, ms_state_dict, dataset_)
...
def clean_accuracy(model: nn.Module,
                   x: torch.Tensor,
                   y: torch.Tensor,
                   batch_size: int = 100,  # 这里改了的，原来是100
    ...
    for counter in range(n_batches):
        ...
        output = model(x_curr)
            # if output.max(1)[1] == y_curr:
            #     print('True')
            # else:
            #     print('False')
            # print('1',output.softmax(1).max(1))
            # print('2',y_curr)
